...
def get_data_for_run(db_config, run_id):
    query = f"""
        SELECT 
            s.step_no, 
            COALESCE(k.total_k, 0) as invest
        FROM steps s
        LEFT JOIN (
    ...
    return pd.DataFrame(res, columns=['step_no', 'invest'])
...
def process_all_runs(db_config, lamb=100000):
    ...
    for rid in run_ids:
        try:
            ...
            #gdp_log = np.log(df_run['invest'].replace(0, np.nan).interpolate())
            gdp_log = df_run['invest'].replace(0, np.nan).interpolate()
